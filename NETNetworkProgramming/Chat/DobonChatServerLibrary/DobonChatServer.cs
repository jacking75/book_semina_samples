...
{
	/// <summary>
	/// Dobon Chat Serverの??を提供する
	/// </summary>
	public class DobonChatServer : TcpChatServer
	{
		#region イベント
		/// <summary>
		/// メンバがログインした
		...
		/// <summary>
		/// DobonChatServerのコンストラク?
		/// </summary>
		public DobonChatServer() : base()
		{
		}
		...
		protected override void OnReceivedData(ReceivedDataEventArgs e)
		{
			...
			if (ClientCommands.Login == cmds[0])
			{
				...
				lock (this._acceptedClients.SyncRoot)
				{
					foreach (AcceptedChatClient c in this._acceptedClients)
					{
						if (nickName == c.Name)
						{
							this.SendErrorMessage(client, "同じ名前のメンバ?がすでにログインしています。");
							return;
						}
					}
			...
			else if (ClientCommands.Logout == cmds[0])
			{
				...
				{
					this.SendErrorMessage(client, "?ャットに参加していません。");
					return;
				}
